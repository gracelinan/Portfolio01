...
var radius = 65;
var radiusScale = d3.scaleLinear().domain([0, 100]).range([25, radius]);
...
var xPositionScale = d3.scalePoint().range([0, width]).padding(0.3);
d3.csv(require('/data/homework-13/all-temps.csv')).then(ready).catch(function (err) {
...
function ready(datapoints) {
  ...
  svg.selectAll('.pie').data(nested).enter().append('g').attr('transform', function (d) {
    var x = xPositionScale(d.key);
    ...
    container.append('path').datum(datapoints).attr('d', line).attr('fill', '#CD5C5C').attr('stroke', 'none').attr('opacity', 0.5);
    container.append('text').text(name).attr('text-anchor', 'middle').attr('alignment-baseline', 'middle').attr('font-weight', '600').style('font-size', 12);
    var bands = [20, 40, 60, 80, 100]; // Draw a circle for each item in bands

    container.selectAll('.band').data(bands).enter().append('circle').attr('fill', 'none').attr('stroke', 'lightgrey').attr('r', function (d) {
      return radiusScale(d);
    }).lower();
    var bandLabels = [20, 60, 100];
    container.selectAll('.label').data(bandLabels).enter().append('text').text(function (d) {
      return d + '°';
    }).attr('y', function (d) {
      return -radiusScale(d);
    }).attr('dy', -2).attr('text-anchor', 'middle').attr('alignment-baseline', 'middle').style('font-size', 8);
  });
  svg.append('text').text('Average Monthly Temperatures').attr('font-weight', '700').attr('text-anchor', 'middle').attr('x', width / 2).attr('y', 20).style('font-size', 26);
  svg.append('text').text('in cities around the world').attr('text-anchor', 'middle').attr('x', width / 2).attr('y', 50).style('font-size', 16);
}
},{"d3":"../node_modules/d3/index.js","/data/homework-13/all-temps.csv":"data/homework-13/all-temps.csv"}]},{},["scripts/homework-13/05-radial-area-multiples.js"], null)
