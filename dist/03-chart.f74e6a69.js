...
d3.csv(require('/data/homework-15/all-temps.csv')).then(ready).catch(function (err) {
...
function ready(datapoints) {
  ...
  var shDatapoints = datapoints.filter(function (d) {
    return d.city === 'Stockholm';
  });
  shDatapoints.push(shDatapoints[0]);
  var lmDatapoints = datapoints.filter(function (d) {
    return d.city === 'Lima';
  });
  lmDatapoints.push(lmDatapoints[0]);
  var tsDatapoints = datapoints.filter(function (d) {
    return d.city === 'Tuscon';
  });
  tsDatapoints.push(tsDatapoints[0]); // const temperatureStore = d3.map(
  //   [
  //     { name: 'NYC' },
  //     { name: 'Lima' },
  //     { name: 'Tuscon' },
  ...
  container.append('path').attr('class', 'temp').datum(nycDatapoints).attr('d', line).attr('fill', '#9ACD32').attr('opacity', 0.75);
  d3.select('#new-york').on('stepin', function () {
    container.select('.temp').datum(nycDatapoints).attr('d', line).attr('fill', '#9ACD32');
  ...
  d3.select('#beijing').on('stepin', function () {
    container.select('.temp').datum(bjDatapoints).attr('d', line).attr('fill', '#F0E68C');
    container.selectAll('.city-name').text('Beijing');
  });
  d3.select('#stockholm').on('stepin', function () {
    container.select('.temp').datum(shDatapoints).attr('d', line).attr('fill', '#DA70D6');
    container.selectAll('.city-name').text('Stockholm');
  });
  d3.select('#lima').on('stepin', function () {
    container.select('.temp').datum(lmDatapoints).attr('d', line).attr('fill', '#FF7F50');
    container.selectAll('.city-name').text('Lima');
  });
  d3.select('#tuscon').on('stepin', function () {
    container.select('.temp').datum(tsDatapoints).attr('d', line).attr('fill', 'purple');
  ...
  function render() {
    ...
    var newRadius = newHeight / 2;
    radiusScale.range([40, newRadius]); // Update things you draw

    svg.select('g').attr('transform', 'translate(' + newWidth / 2 + ',' + newHeight / 2 + ')');
    container.select('.temp').datum(nycDatapoints).attr('d', line);
    container.selectAll('.bands').data(circleBands).attr('r', function (d) {
      return radiusScale(d);
    });
    container.selectAll('.temp-notes').data(textBands).attr('y', function (d) {
      return -radiusScale(d);
    }); // Update axes
...
},{"d3":"../node_modules/d3/index.js","/data/homework-15/all-temps.csv":"data/homework-15/all-temps.csv"}]},{},["scripts/homework-15/03-chart.js"], null)
