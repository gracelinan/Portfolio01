...
function ready(_ref) {
  ...
  container.selectAll('svg').data(nested).enter().append('svg').attr('height', height + margin.top + margin.bottom).attr('width', width + margin.left + margin.right).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')').each(function (d) {
    ...
    svg.append('text').text(name.charAt(0).toUpperCase() + name.slice(1)).attr('x', width / 2) // in the center
    .attr('y', height / 2).attr('text-anchor', 'middle') // center aligned
    .attr('font-size', 15);
  });
}
},{"d3":"../node_modules/d3/index.js","topojson":"../node_modules/topojson/index.js","/data/homework-17/us_states.topojson":"data/homework-17/us_states.topojson","/data/homework-17/powerplants.csv":"data/homework-17/powerplants.csv"}]},{},["scripts/homework-17/06-map.js"], null)
