...
d3.csv(require('/data/homework-15/housing-prices.csv')).then(ready).catch(function (err) {
  console.log(err);
});
...
function ready(datapoints) {
  ...
  d3.select('#drawLines').on('stepin', function () {
    svg.selectAll('.region-line').attr('stroke', function (d) {
      return colorScale(d.key);
    }).style('visibility', 'visible');
    svg.selectAll('.region-circle').attr('fill', function (d) {
      return colorScale(d.key);
    }).style('visibility', 'visible');
    svg.selectAll('.path-text').attr('fill', 'black').attr('font-weight', '400').style('visibility', 'visible');
    svg.select('rect').style('visibility', 'hidden');
  });
  d3.select('#highlightUS').on('stepin', function () {
    ...
    svg.selectAll('.path-text').attr('fill', function (d) {
      if (d.key === 'U.S.') {
        return 'red';
      } else {
        return 'lightgrey';
  ...
  svg.selectAll('path').data(nested).enter().append('path').attr('class', 'region-line') // .attr('class', d => {
  ...
  svg.selectAll('circle').data(nested).enter().append('circle').attr('class', 'region-circle') // .attr('class', d => {
  //   return d.key.toLowerCase().replace(/[^a-z]*/g, '')
  // })
  ...
  svg.selectAll('text').data(nested).enter().append('text').attr('class', 'path-text') // .attr('class', d => {
  //   return d.key.toLowerCase().replace(/[^a-z]*/g, '')
  // })
  .attr('y', function (d) {
    return yPositionScale(d.values[0].price);
  ...
  svg.append('text').attr('font-size', '24').attr('text-anchor', 'middle').text('U.S. housing prices fall in winter').attr('class', 'title').attr('x', width / 2).attr('y', -40).attr('dx', 40);
  var rectWidth = xPositionScale(parseTime('February-17')) - xPositionScale(parseTime('November-16'));
  svg.append('rect').attr('x', xPositionScale(parseTime('December-16'))).attr('y', 0).attr('width', rectWidth).attr('height', height).attr('fill', '#C2DFFF').lower();
  ...
  function render() {
    ...
    svg.selectAll('path').data(nested).attr('d', function (d) {
      return line(d.values);
    });
    svg.selectAll('circle').data(nested).attr('cy', function (d) {
      return yPositionScale(d.values[0].price);
    }).attr('cx', function (d) {
      return xPositionScale(d.values[0].datetime);
    });
    svg.selectAll('.path-text').data(nested).attr('y', function (d) {
      return yPositionScale(d.values[0].price);
    }).attr('x', function (d) {
      return xPositionScale(d.values[0].datetime);
    });
    svg.select('.title').attr('x', newWidth / 2);
    var rectNewWidth = xPositionScale(parseTime('February-17')) - xPositionScale(parseTime('November-16'));
    svg.select('rect').attr('x', xPositionScale(parseTime('December-16'))).attr('y', 0).attr('width', rectNewWidth).attr('height', newHeight); // Update axes
...
},{"d3":"../node_modules/d3/index.js","/data/homework-15/housing-prices.csv":"data/homework-15/housing-prices.csv"}]},{},["scripts/homework-15/02-chart.js"], null)
