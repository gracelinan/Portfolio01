...
var width = 175 - margin.left - margin.right; // I'll give you this part!

var container = d3.select('#chart-04'); // Create your scales

var xPositionScale = d3.scaleLinear().domain([2015, 2019]).range([0, width]);
...
d3.csv(require('/data/project-03/populous-country.csv')).then(ready); // Create your ready function

function ready(datapoints) {
  var percentages = datapoints.map(function (d) {
    return +d.percentage;
  });
  var percentageMax = d3.max(percentages);
  ...
  container.selectAll('svg').data(nested).enter().append('svg').attr('height', height + margin.top + margin.bottom).attr('width', width + margin.left + margin.right).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')').each(function (d) {
    var name = d.key;
    var datapoints = d.values; // console.log(datapoints)
    // What SVG are we in? Let's grab it.

    var svg = d3.select(this);
    svg.append('path').datum(datapoints).attr('stroke', '#CD5C5C').attr('fill', 'none').attr('d', line).attr('stroke-width', 2);
    svg.append('text').text(name).attr('x', width / 2) // in the center
    .attr('text-anchor', 'middle') // center aligned
    .attr('dy', -10).attr('font-size', 12).attr('fill', '#CD5C5C').style('font-weight', 'bold');
    var xAxis = d3.axisBottom(xPositionScale).ticks(5) // .tickValues([2015, 2017, 2019])
    .tickFormat(d3.format('d')).tickSize(-height);
    svg.append('g').attr('class', 'axis x-axis').attr('transform', 'translate(0,' + height + ')').call(xAxis).selectAll('.tick line').attr('stroke-dasharray', '2 2');
    var yAxis = d3.axisLeft(yPositionScale).tickValues([1.5, 3, 4.5, 6]) // .tickFormat(d3.format('d, %'))
    .tickSize(-width);
    svg.append('g').attr('class', 'axis y-axis').call(yAxis).selectAll('.tick line').attr('stroke-dasharray', '2 2');
    svg.selectAll('.domain').remove();
  });
}
},{"d3":"../node_modules/d3/index.js","/data/project-03/populous-country.csv":"data/project-03/populous-country.csv"}]},{},["scripts/project-03/chart-04.js"], null)
//# sourceMappingURL=/chart-04.59cab903.js.map