...
var svg = d3.select('#chart-2 ').append('svg').attr('height', height + margin.top + margin.bottom).attr('width', width + margin.left + margin.right).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')'); // .attr('transform', `translate(${width / 2},${height / 2})`)

var radius = 80;
var xScale = d3.scalePoint().domain(['Project 1', 'Project 2', 'Project 3', 'Project 4']).range([radius, width - radius]);
var colorScale = d3.scaleOrdinal().range(['#7fc97f', '#beaed4', '#fdc086']);
var pie = d3.pie().value(function (d) {
  return d.minutes;
});
var arc = d3.arc().innerRadius(0).outerRadius(radius);
d3.csv(require('/data/homework-13/time-breakdown-all.csv')).then(ready).catch(function (err) {
...
function ready(datapoints) {
  ...
  svg.selectAll('.pie').data(nested).enter().append('g').attr('transform', function (d) {
    ...
    container.append('text').text(name).attr('text-anchor', 'middle') // center aligned
    .attr('y', 100); // .attr('font-size', 12)
  });
}
},{"d3":"../node_modules/d3/index.js","/data/homework-13/time-breakdown-all.csv":"data/homework-13/time-breakdown-all.csv"}]},{},["scripts/homework-13/02-pie-multiples.js"], null)
