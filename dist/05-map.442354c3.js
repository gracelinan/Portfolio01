...
},{"topojson-client":"../node_modules/topojson/node_modules/topojson-client/index.js","topojson-server":"../node_modules/topojson/node_modules/topojson-server/index.js","topojson-simplify":"../node_modules/topojson/node_modules/topojson-simplify/index.js"}],"data/homework-17/us_states.topojson":[function(require,module,exports) {
module.exports = "/us_states.660447ae.topojson";
},{}],"data/homework-17/powerplants.csv":[function(require,module,exports) {
module.exports = "/powerplants.b3256a30.csv";
...
var projection = d3.geoAlbersUsa().scale(700);
var path = d3.geoPath().projection(projection);
var colorScale = d3.scaleOrdinal(d3.schemeSet1);
var radiusScale = d3.scaleSqrt().range([0, 15]);
var bandScale = d3.scaleBand().range([100, height - 100]);
Promise.all([d3.json(require('/data/homework-17/us_states.topojson')), d3.csv(require('/data/homework-17/powerplants.csv'))]).then(ready).catch(function (err) {
...
function ready(_ref) {
  ...
  svg.selectAll('circle').data(datapoints).enter().append('circle').attr('transform', function (d) {
    var coords = [d.Longitude, d.Latitude]; // coords is our long/lat pair
    // console.log(coords)
  ...
  }).attr('opacity', 0.5); // console.log(states.features)
  ...
  }).attr('text-anchor', 'middle').attr('alignment-baseline', 'middle').attr('font-size', 12); // .style(
  //   'text-shadow',
  //   '-1px -1px 0 #fff, 1px -1px 0 #fff, -1px 1px 0 #fff, 1px 1px 0 #fff'
  // )

  bandScale.domain(colorScale.domain());
  svg.selectAll('g').data(colorScale.domain()).enter().append('g').each(function (d) {
    // console.log(d)
    var g = d3.select(this);
    g.append('circle').attr('r', 10).attr('cx', -120) // .attr('cy', 100)
    .attr('cy', function (d) {
      return bandScale(d);
    }).attr('fill', function (d) {
      return colorScale(d);
    });
    g.append('text').text(d.charAt(0).toUpperCase() + d.slice(1)).attr('x', -100) // .attr('text-anchor', 'middle') // center aligned
    .attr('y', function (d) {
      return bandScale(d);
    }).attr('font-size', 12);
  });
}
},{"d3":"../node_modules/d3/index.js","topojson":"../node_modules/topojson/index.js","/data/homework-17/us_states.topojson":"data/homework-17/us_states.topojson","/data/homework-17/powerplants.csv":"data/homework-17/powerplants.csv"}]},{},["scripts/homework-17/05-map.js"], null)
