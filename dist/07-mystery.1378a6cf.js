...
var svg = d3.select('#chart-7').append('svg').attr('height', height + margin.top + margin.bottom).attr('width', width + margin.left + margin.right).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')').append('g').attr('transform', "translate(".concat(width / 2, ",").concat(height / 2, ")")); // Create a time parser

var parseTime = d3.timeParse('%H-%M');
var angleScale = d3.scaleBand().range([0, Math.PI * 2]);
var radius = 150;
var radiusScale = d3.scaleLinear().domain([0, 5]).range([0, radius]);
var colorScale = d3.scaleOrdinal().range(['blue', 'brown']);
var line = d3.radialArea().angle(function (d) {
  return angleScale(d.timeParse);
}).innerRadius(0).outerRadius(function (d) {
  return radiusScale(d.total);
}); // .innerRadius(d => radiusScale(d.total))
// .outerRadius(200)

d3.csv(require('/data/homework-13/time-binned.csv')).then(ready).catch(function (err) {
...
function ready(datapoints) {
  ...
  var times = datapoints.map(function (d) {
    return d.timeParse;
  });
  angleScale.domain(times);
  datapoints.push(datapoints[0]);
  svg.append('path').datum(datapoints).attr('d', line).attr('fill', 'none').attr('stroke', 'black');
  svg.append('circle').datum(datapoints).attr('r', 3).attr('fill', 'grey').attr('cx', angleScale(parseTime(d.time))).attr('cy', function (d) {
    console.log(d);
    return radiusScale(d.total);
  });
  ...
  svg.selectAll('.band').data(bands).enter().append('circle').attr('fill', 'none').attr('stroke', 'lightgrey').attr('r', radius).lower();
}
},{"d3":"../node_modules/d3/index.js","/data/homework-13/time-binned.csv":"data/homework-13/time-binned.csv"}]},{},["scripts/homework-13/07-mystery.js"], null)
