...
var colorScale = d3.scaleLinear().domain([0, 100]).range(['#87CEFA', 'pink']);
var xScale = d3.scalePoint().range([0, width]).padding(0.2);
...
function ready(datapoints) {
  ...
  svg.selectAll('.pie').data(nested).enter().append('g').attr('transform', function (d) {
    ...
    container.selectAll('path').data(pie(datapoints)).enter().append('path').attr('fill', function (d) {
      return colorScale(d.data.high_temp);
    }).attr('d', function (d) {
      return arc(d);
    });
    container.append('circle').attr('r', 1).attr('fill', 'grey');
    container.append('text').text(name).attr('text-anchor', 'middle').attr('y', 100); // .style('font-size', 22)
  });
}
},{"d3":"../node_modules/d3/index.js","/data/homework-13/all-temps.csv":"data/homework-13/all-temps.csv"}]},{},["scripts/homework-13/03c-donut-wedges-multiples.js"], null)
