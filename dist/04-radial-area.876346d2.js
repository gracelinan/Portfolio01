...
d3.csv(require('/data/homework-13/ny-temps.csv')).then(ready).catch(function (err) {
  return console.log('Failed on', err);
});

function ready(datapoints) {
  datapoints.push(datapoints[0]);
  svg.append('path').datum(datapoints).attr('d', line).attr('fill', '#00CED1').attr('stroke', 'none').attr('opacity', 0.5);
  svg.append('text').text('NYC').attr('text-anchor', 'middle').attr('alignment-baseline', 'middle').attr('font-weight', '600').style('font-size', 26);
  var bands = [20, 30, 40, 50, 60, 70, 80, 90]; // Draw a circle for each item in bands

  svg.selectAll('.band').data(bands).enter().append('circle').attr('fill', 'none').attr('stroke', 'lightgrey').attr('r', function (d) {
    console.log(d);
    return radiusScale(d);
  }).lower();
  var bandLabels = [30, 50, 70, 90];
  svg.selectAll('.label').data(bandLabels).enter().append('text').text(function (d) {
    return d + '°';
  }).attr('y', function (d) {
    return -radiusScale(d);
  }).attr('dy', -5).attr('text-anchor', 'middle').attr('alignment-baseline', 'middle').style('font-size', 10);
}
},{"d3":"../node_modules/d3/index.js","/data/homework-13/ny-temps.csv":"data/homework-13/ny-temps.csv"}]},{},["scripts/homework-13/04-radial-area.js"], null)
