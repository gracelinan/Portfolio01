...
var colorScale = d3.scaleLinear().domain([0, 85]).range(['#87CEFA', 'pink']);
...
function ready(datapoints) {
  ...
  svg.selectAll('path').data(pie(datapoints)).enter().append('path').attr('fill', function (d) {
    return colorScale(d.data.high_temp);
  }).attr('d', function (d) {
    return arc(d);
  });
  svg.append('circle').attr('r', 2).attr('fill', 'grey');
  svg.append('text').text('NYC high temperatures, by month').attr('font-weight', '600').attr('text-anchor', 'middle').attr('y', -100).style('font-size', 22);
}
},{"d3":"../node_modules/d3/index.js","/data/homework-13/ny-temps.csv":"data/homework-13/ny-temps.csv"}]},{},["scripts/homework-13/03-wedges.js"], null)
