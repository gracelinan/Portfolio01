...
d3.csv(require('/data/homework-13/ratings.csv')).then(ready).catch(function (err) {
...
function ready(datapoints) {
  ...
  svg.append('path').datum(datapoints).attr('d', line).attr('fill', 'pink').attr('stroke', 'black').attr('opacity', 0.5);
  svg.append('circle').attr('r', 3).attr('fill', 'grey');
  var bands = [0.5, 1, 1.5, 2, 2.5, 3, 3.5, 4, 4.5, 5]; // Draw a circle for each item in bands

  svg.selectAll('.band').data(bands).enter().append('circle').attr('fill', 'none').attr('stroke', 'lightgrey').attr('r', function (d) {
    return radiusScale(d);
  }).lower();
  svg.selectAll('.radius-line').data(angleScale.domain()).enter().append('line').attr('x1', 0).attr('y1', 0).attr('x2', 0).attr('y2', -radius).attr('stroke', 'lightgrey') // .style/css knows about radians
  ...
  svg.selectAll('.outside-label').data(angleScale.domain()).enter().append('text').attr('font-weight', '600').text(function (d) {
    return d;
  }).attr('y', -radius) // set it up at the top of the chart
  .attr('dy', -10) // give a little offset to push it higher
  .attr('transform', function (d) {
    return "rotate(".concat(angleScale(d) * 180 / Math.PI, ")");
  }).attr('text-anchor', 'middle').attr('alignment-baseline', 'middle'); // .attr('x', function(d) {
...
},{"d3":"../node_modules/d3/index.js","/data/homework-13/ratings.csv":"data/homework-13/ratings.csv"}]},{},["scripts/homework-13/06-radar.js"], null)
