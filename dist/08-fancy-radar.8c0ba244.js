...
var svg = d3.select('#chart-8').append('svg').attr('height', height + margin.top + margin.bottom).attr('width', width + margin.left + margin.right).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')').append('g').attr('transform', "translate(".concat(width / 2, ",").concat(height / 2, ")"));
var angleScale = d3.scaleBand().range([0, Math.PI * 2]);
var radius = 150;
var radiusScale = d3.scaleLinear().domain([0, 10]).range([0, radius]);
var line = d3.radialArea().angle(function (d) {
  return angleScale(d.name);
}).innerRadius(0).outerRadius(function (d) {
  return radiusScale(d.value);
});
d3.csv(require('/data/homework-13/nba.csv')).then(ready).catch(function (err) {
  return console.log('Failed with', err);
});

function ready(datapoints) {
  var player = datapoints[0];
  var maxMinutes = 60;
  var maxPoints = 30;
  var max3Point = 5;
  var maxRebounds = 15;
  ...
  var customDatapoints = [{
    name: 'Minutes',
    value: player.MP / maxMinutes * 10
  }, {
    name: 'Points',
    value: player.PTS / maxPoints * 10
  }, {
    name: 'Field Goals',
    value: player.FG
  }, {
    name: '3-Point Field Goals',
    value: player['3P'] / max3Point * 10
  }, {
    name: 'Free Throws',
    value: player.FT
  }, {
    name: 'Rebounds',
    value: player.TRB / maxRebounds * 10
  }, {
    name: 'Assists',
    value: player.AST
  }, {
    name: 'Steals',
    value: player.STL / maxSteals * 10
  }, {
    name: 'Blocks',
    value: player.BLK / maxBlocks * 10
  }];
  var categories = customDatapoints.map(function (d) {
    return d.name;
  });
  angleScale.domain(categories);
  customDatapoints.push(customDatapoints[0]);
  var bands = [2, 4, 6, 8, 10]; // Draw a circle for each item in bands
  ...
  }).attr('mask', 'url(#playerMask)').lower();
  svg.selectAll('.band').data(bands).enter().append('circle').attr('fill', 'none').attr('stroke', 'lightgrey').attr('r', function (d) {
    return radiusScale(d);
  }).attr('fill', function (d, i) {
    console.log('Looking at circle number', i % 2);
  ...
  svg.append('mask').attr('id', 'playerMask').append('path').datum(customDatapoints).attr('d', line).attr('fill', 'white').attr('stroke', 'white');
  svg.append('text').text('LeBron James').attr('font-weight', '450').attr('text-anchor', 'middle').attr('y', -200).style('font-size', 20);
  svg.append('text').text('Cleveland Cavaliers').attr('font-weight', '400').attr('text-anchor', 'middle').attr('y', -185).style('font-size', 14);
}
},{"d3":"../node_modules/d3/index.js","/data/homework-13/nba.csv":"data/homework-13/nba.csv"}]},{},["scripts/homework-13/08-fancy-radar.js"], null)
